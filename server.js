...
app.post("/verify", async (req, res) => {
  ...
  const DELAI_ENTRE_TENTATIVES_MS = 4000;

  for (let tentative = 1; tentative <= NOMBRE_TENTATIVES; tentative++) {
    try {
      const transaction = await k.verify(transactionId);
      console.log(`Tentative ${tentative} — statut reçu : ${transaction.status}`);

      const estReussi = transaction.status === "SUCCESS";
      const estEnAttente = transaction.status === "PENDING" || transaction.status === "PROCESSING";
      ...
      if (estEnAttente && tentative < NOMBRE_TENTATIVES) {
      ...
      const estTransactionIntrouvable = error.message && error.message.includes("Not Found");

      if (estTransactionIntrouvable && tentative < NOMBRE_TENTATIVES) {
        console.log(`Transaction pas encore trouvée, nouvelle tentative dans ${DELAI_ENTRE_TENTATIVES_MS / 1000}s (essai ${tentative}/${NOMBRE_TENTATIVES})`);
        await attendre(DELAI_ENTRE_TENTATIVES_MS);
        continue;
      }
